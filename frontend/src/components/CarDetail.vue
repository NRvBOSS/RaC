<template>
  <div v-if="car" class="pb-20 bg-gray-50 min-h-screen">
    <!-- Header info -->
    <div
      class="bg-gradient-to-r from-gray-900 to-gray-800 text-white py-8 px-6 shadow-xl"
    >
        ...
        <div class="flex justify-between items-start">
          <div>
            ...
            <div class="flex flex-wrap items-center gap-3">
              <span
                class="bg-amber-500/90 text-gray-900 px-3 py-1 rounded-full text-sm font-bold"
                >{{ car.year }}</span
              >
              <span
                class="bg-gray-700 text-white px-3 py-1 rounded-full text-sm font-medium"
                >{{ car.engine }}</span
              >
              <span
                v-if="car.city"
                class="bg-gray-700 text-white px-3 py-1 rounded-full text-sm font-medium flex items-center"
              >
                <svg
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
      <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="lg:w-2/3">
          ...
          >
            <img
              ...
              :src="
                car.image ||
                'https://via.placeholder.com/800x500?text=Premium+Vehicle'
              "
              :alt="car.name"
            />
          </div>
          ...
          <!-- Specifications -->
          <div
            class="mt-8 bg-white rounded-xl shadow-md overflow-hidden border border-gray-200"
          >
        ...
        <div class="lg:w-1/3">
          <div
            class="bg-white rounded-xl shadow-xl p-6 sticky top-6 border border-gray-200"
          >
            <div class="mb-6">
              ...
              <p v-if="owner.name" class="text-gray-600 mt-2 text-sm">
                <span class="font-medium">Owner:</span> {{ owner.name }}
              </p>
            </div>
            ...
            <div class="relative group">
              ...
              >
                ...
                >
                  ...
                  <span class="text-base font-bold">{{ owner.phone }}</span>
                </div>
              </button>
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
      ...
      >
        ...
        <div class="px-6 py-4">
          <p v-if="car.description" class="text-gray-700">
            {{ car.description }}
          </p>
          <p v-else class="text-gray-500 italic">
            No detailed description available for this vehicle.
          </p>
        </div>
      </div>
    </div>
  ...
  >
    <div
      class="text-center p-8 max-w-2xl mx-auto bg-white rounded-xl shadow-md border border-gray-200"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        class="h-16 w-16 mx-auto text-amber-600"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
      >
  ...
  try {
    ...
    if (response.data.owner) {
      const ownerResponse = await axios.get(
        `http://localhost:4000/api/user/${response.data.owner}`
      );
      owner.value = {
